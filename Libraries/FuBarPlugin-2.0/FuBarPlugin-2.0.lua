...
function MinimapContainer:AddPlugin(plugin)
	...
	if not plugin.minimapFrame then
		...
		frame.self = plugin
		if not frame_OnEnter then
			function frame_OnEnter(self)
				self = self or this
				if self and type(self.self.OnEnter) == "function" then
					self.self:OnEnter()
				end
			end
		end
		frame:SetScript("OnEnter", frame_OnEnter)
		if not frame_OnLeave then
			function frame_OnLeave(self)
				self = self or this
				if self and type(self.self.OnLeave) == "function" then
					self.self:OnLeave()
				end
			end
		end
		frame:SetScript("OnLeave", frame_OnLeave)
		if not frame_OnClick then
			function frame_OnClick(self, button)
				self = self or this
				if not self then return end
				if self.self:IsMinimapAttached() and self.dragged then return end
				if type(self.self.OnClick) == "function" then
					self.self:OnClick(button or arg1)
				end
			end
		end
		frame:SetScript("OnClick", frame_OnClick)
		if not frame_OnDoubleClick then
			function frame_OnDoubleClick(self, button)
				self = self or this
				if self and type(self.self.OnDoubleClick) == "function" then
					self.self:OnDoubleClick(button or arg1)
				end
			end
		end
		frame:SetScript("OnDoubleClick", frame_OnDoubleClick)
		if not frame_OnReceiveDrag then
			function frame_OnReceiveDrag(self)
				self = self or this
				if self and (self.self:IsMinimapAttached() and not self.dragged) and type(self.self.OnReceiveDrag) == "function" then
					self.self:OnReceiveDrag()
				end
			end
		end
		frame:SetScript("OnReceiveDrag", frame_OnReceiveDrag)
		if not minimap_OnMouseDown then
			function minimap_OnMouseDown(self, button)
				self = self or this
				if not self then return end
				local clickedButton = button or arg1
				self.dragged = false
				if clickedButton == "LeftButton" and not IsShiftKeyDown() and not IsControlKeyDown() and not IsAltKeyDown() then
					HideDropDownMenu(1)
					if type(self.self.OnMouseDown) == "function" then
						self.self:OnMouseDown(clickedButton)
					end
				elseif clickedButton == "RightButton" and not IsShiftKeyDown() and not IsControlKeyDown() and not IsAltKeyDown() then
					self.self:OpenMenu(self)
				else
					HideDropDownMenu(1)
					if type(self.self.OnMouseDown) == "function" then
						self.self:OnMouseDown(clickedButton)
				...
				if self.self.OnClick or self.self.OnMouseDown or self.self.OnMouseUp or self.self.OnDoubleClick then
					if self.self.minimapIcon:GetTexture():sub(1, 16) == "Interface\\Icons\\" then
						self.self.minimapIcon:SetTexCoord(0.14, 0.86, 0.14, 0.86)
					else
						self.self.minimapIcon:SetTexCoord(0.1, 0.9, 0.1, 0.9)
					end
				end
			end
		end
		...
		if not minimap_OnMouseUp then
			function minimap_OnMouseUp(self, button)
				self = self or this
				if not self then return end
				if not self.dragged and type(self.self.OnMouseUp) == "function" then
					self.self:OnMouseUp(button or arg1)
				end
				if self.self.minimapIcon:GetTexture():sub(1, 16) == "Interface\\Icons\\" then
					self.self.minimapIcon:SetTexCoord(0.05, 0.95, 0.05, 0.95)
				else
					self.self.minimapIcon:SetTexCoord(0, 1, 0, 1)
				end
			end
		end
...
function MinimapContainer.OnDragStart(self)
	self = self or this
	if not self then return end
	self.dragged = true
	self:LockHighlight()
...
function MinimapContainer.OnDragStop(self)
	self = self or this
	if not self then return end
...
function MinimapContainer.OnUpdate(self)
	self = self or this
	if not self then return end
	if not IsAltKeyDown() then
		...
		if self.self.db then
			self.self.db.profile.minimapPosition = position
			self.self.db.profile.minimapPositionX = nil
			self.self.db.profile.minimapPositionY = nil
			self.self.db.profile.minimapPositionWild = nil
		else
			self.self.minimapPosition = position
			self.self.minimapPositionX = nil
			self.self.minimapPositionY = nil
			self.self.minimapPositionWild = nil
		end
	else
		local px, py = GetCursorPosition()
		local scale = UIParent:GetEffectiveScale()
		px, py = px / scale, py / scale
		if self.self.db then
			self.self.db.profile.minimapPositionX = px
			self.self.db.profile.minimapPositionY = py
			self.self.db.profile.minimapPosition = nil
			self.self.db.profile.minimapPositionWild = true
		else
			self.self.minimapPositionX = px
			self.self.minimapPositionY = py
			self.self.minimapPosition = nil
			self.self.minimapPositionWild = true
		end
	end
	MinimapContainer:ReadjustLocation(self.self)
end
